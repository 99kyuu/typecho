...
mysql -u$db_user -p$db_pass $db_name -e "LOAD DATA INFILE \"${file_relationship}\" into table ${tb_relationships} character set utf8 FIELDS  TERMINATED BY \"${split_char}\"";
echo "done for ${tb_relationships}"
mysql -u$db_user -p$db_pass $db_name -e "LOAD DATA INFILE \"${file_meta}\" into table ${tb_meta} character set utf8 FIELDS  TERMINATED BY \"${split_char}\"";
echo "done for ${tb_meta}"
mysql -u$db_user -p$db_pass $db_name -e "LOAD DATA INFILE \"${file_content}\" into table ${tb_content} character set utf8 FIELDS  TERMINATED BY \"${split_char}\"";
echo "done for ${tb_content}"
mysql -u$db_user -p$db_pass $db_name -e "LOAD DATA INFILE \"${file_content_ext}\" into table ${tb_content_ext} character set utf8 FIELDS  TERMINATED BY \"${split_char}\"";
echo "done for ${tb_content_ext}"
mysql -u$db_user -p$db_pass $db_name -e "LOAD DATA INFILE \"${file_content_ext}\" into table ${tb_content_index} character set utf8 FIELDS  TERMINATED BY \"${split_char}\"";

...
class Widget_Archive extends Widget_Abstract_Contents
{
    ...
    private function tagHandle(Typecho_Db_Query $select, &$hasPushed)
    {
        ...
        if(empty($test_result)){
            ...
            //通过全文搜索的方式检索，而且仅检索当前分类，不检索子分类
            $select->where('match(table.contents.ext_categories) against( ? in boolean mode)', sprintf("%04d",intval($tag['mid'])))
                ->where('table.contents.type = ?', 'post');
            //echo $select;die();
        }
    ...
    public function execute()
    {
        ...
        if(defined('OPTIMIZE_ORDER_BY') && OPTIMIZE_ORDER_BY == true){
